...
class Step_by_step_survival():
    ...
    def Model_load(self):
        if self.model_load_path:
            ...
            # 看究竟是用来预测，还是用来计算Radscore
            try:
                self.labeled_trained_model_data = pickle.load(open(self.model_load_path, 'rb'))[0] #载入预测模型的data,包括imp和scaler
                model = pickle.load(open(self.model_load_path, 'rb'))[1] #载入的模型类
                self.preprocessing = pickle.load(open(self.model_load_path, 'rb'))[2] #数据集预处理的过程，即进行何种缺失值填补、何种数据标准化
                self.train_data_for_radscore = pickle.load(open(self.model_load_path, 'rb'))[3] # 载入r用于预测
                self.features = model.data_variable_names #模型中使用的特征名称
    ...
    def data_predict_(self):
        self.labeled_data = pd.read_csv(self.Data_predict_path,engine = 'python') #读入需要被预测的数据
        # 如果是用来预测结局
        if self.Predict_module_predict:
            ...
            self.labeled_data_after_transform = pd.DataFrame(data, columns=self.labeled_data.columns[2:])   # 数据已经预处理完毕，此时为pd.DataFrame
    ...
    def Whole_report(self):
        ...
        for i in np.arange(1, self.operative_times + 1):
            print('self.operative_process[i][0]',self.operative_process[i][0])
            if self.operative_process[i][0] == "machine_learning":
                # 提取机器学习步骤，名称，准确率，AUC；机器学习步骤便于追踪最佳模型前面的步骤
        ...
        if is_machine_learning:
            ...
            pass
        # 如果已经进行机器学习，则获取最优模型之前的特征筛选步骤；如果没有进行机器学习，则不输出summary report
        if is_machine_learning:
...
#                        report_table2 = word.add_table(rows = variables.size + 1, cols = 7, style = "Table Grid")
                        report_table2.autofit = False
                        report_table2.columns[0].width = Inches(0.1)
#                        report_table2.cell(0,0).text = columns[:2]
...
#                        report_table2.cell(b,0).text = variable_test[b-1][:4]
