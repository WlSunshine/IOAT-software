...
def return_range_for_heat_figure_survival(train_feature, *test_feature): 
    if test_feature:
...
def report_heat_figure(data, title = "Clustermap of the features", rotation = 45, vmin=None, vmax=None):
    # Set whether to display axis coordinates
    if (data.shape[0] <= 20):
    ...
    # Set the label scale range, set the vmin and vmax of the function
...
def fix_abnormal_nan_survival(the_data, the_mean=False, the_std=False):
    ...
    intermediate_data_finally = intermediate_data_finally.T
    return(intermediate_data_finally, the_used_mean, the_used_std)
# Feature Screening Correlation Coefficient Graph
def correlation_analysis_survival(feature, is_train=True):
    if len(feature.columns.values.tolist()) <= 10:
        # Define the location of the output graphics
        filename = "\\".join([dirname.name, "".join([str(time.time()), "figure.png"])])
        # Calculate the correlation coefficient
        corr_matrix = feature.corr() # pearson Correlation coefficient
        # corr_matrix = feature.corr("kendall") # Kendall Tau
        # corr_matrix = feature.corr("spearman") # spearman
        # Heat map display and save
        fig, ax = plt.subplots(figsize=(9,9)) # Set the screen size
        ...
        plt.show()
        return(filename)
# Define the correlation between independent variables to filter variables
def correlation_xx_survival(feature_data, cutoff):
    ...
    return(remaincol)
# Use R to find lasso_cox, filter variables
...
# According to the data exported by R, draw a lasso diagram
def Lasso_cox_survival(data_path, data_path_best):
    data = pd.read_csv(data_path)
    data_value = data.values
    data1 = data_value[:, -2:]       # Read cvm data
    data2 = data_value[:, :-2]       # Read coefficients data
...
class Feature_selection_survival(object):
    ...
    def select_by_lasso_cox(self):
        ...
        # Make the legend labels of the two cluster heat maps consistent
        if self.test:
    ...
    def select_by_stepwise_cox(self, select = True, radscore = False):
        ...
        if radscore == True:
            ...
            self.data_variable_names_for_radscore = np.array(coefficients.values[:,0]) # Output the variable name used to calculate radscore, which is used in the model prediction module
            self.coef_after_radscore = coefficients_values # Output and calculate the coefficient of radscore, which is used in the model prediction module
